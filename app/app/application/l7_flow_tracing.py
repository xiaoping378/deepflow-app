...
class L7FlowTracing(Base):
    ...
    async def trace_l7_flow(self,
        ...
        for i in range(max_iteration):
            ...
            if self.signal_sources != ['otel']:
                ...
                if filters:
                    ...
                    new_related_map = defaultdict(set)
                    if xrequests:
            ...
            if old_flows_length == new_flows_length:
                break
        set_all_relate(dataframe_flowmetas, related_map)
        if not l7_flow_ids:
            return {}
        # 获取追踪到的所有应用流日志
        ...
        for index in range(len(l7_flows.index)):
            l7_flows["related_ids"][index] = list(
                related_map[l7_flows._id[index]])
        # 对所有应用流日志排序
    ...
    def set_relate(self, df, related_map):
        for i in range(len(df.index)):
            ...
            if self.x_request_id_0 and self.x_request_id_0 == df.x_request_id_1[
                    i]:
                related_map[df._id[i]].add(self._id)
                continue
...
def sort_all_flows(dataframe_flows: DataFrame, network_delay_us: int,
    ...
    for flow in flows:
        ...
        for related_id in flow["related_ids"]:
            ...
            if related_id in flow["_id"]:
                continue
            if id_map.get(related_id, None) is not None:
                related_ids.append(f"{id_map[related_id]}-{related_id}")
